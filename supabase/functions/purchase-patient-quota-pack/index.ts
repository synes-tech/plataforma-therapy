...
import { PurchasePatientQuotaPackSchema } from './schema.ts';
import { purchasePatientQuotaPack } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('patient_addon.attempt', user, 'purchase-patient-quota-pack');

    const body = await req.json().catch(() => ({}));
    const parsed = PurchasePatientQuotaPackSchema.safeParse(body);

    if (!parsed.success) {
      return errorResponse(new ValidationError(parsed.error.flatten().fieldErrors), req);
    }

    const result = await purchasePatientQuotaPack(parsed.data, user);
