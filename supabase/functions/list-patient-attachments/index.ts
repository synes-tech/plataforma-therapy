...
import { ListPatientAttachmentsSchema } from './schema.ts';
import { listPatientAttachments } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('list_patient_attachments.attempt', user, 'list-patient-attachments');

    const body = await req.json();
    const parsed = ListPatientAttachmentsSchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse(new ValidationError(parsed.error.flatten().fieldErrors), req);
    }

    const result = await listPatientAttachments(parsed.data, user);
