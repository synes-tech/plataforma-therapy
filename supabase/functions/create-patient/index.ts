...
import { CreatePatientSchema } from './schema.ts';
import { createPatient } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('create_patient.attempt', user, 'create-patient');
    ...
    const result = await createPatient(parseResult.data, user, '');
