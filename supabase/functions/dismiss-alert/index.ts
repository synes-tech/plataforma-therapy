...
import { DismissAlertSchema } from './schema.ts';
import { dismissAlert } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('dismiss_alert.attempt', user, 'dismiss-alert');
    ...
    const result = await dismissAlert(user, parseResult.data.diary_entry_id);
