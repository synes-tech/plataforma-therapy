...
import { DeletePatientAttachmentSchema } from './schema.ts';
import { deletePatientAttachment } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('delete_patient_attachment.attempt', user, 'delete-patient-attachment');

    const body = await req.json();
    const parsed = DeletePatientAttachmentSchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse(new ValidationError(parsed.error.flatten().fieldErrors), req);
    }

    const result = await deletePatientAttachment(parsed.data, user);
