...
import { GetPatientSessionsSchema } from './schema.ts';
import { getPatientSessions } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('get_patient_sessions.attempt', user, 'get-patient-sessions');

    const body = await req.json();
    const parseResult = GetPatientSessionsSchema.safeParse(body);

    if (!parseResult.success) {
      return errorResponse(new ValidationError(parseResult.error.flatten().fieldErrors), req);
    }

    const result = await getPatientSessions(parseResult.data, user);
