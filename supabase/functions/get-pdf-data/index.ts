...
import { PdfExportSchema } from './schema.ts';
import { getPdfData } from './service.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    logAuthEvent('pdf_export.data_fetch', user, 'get-pdf-data');
    ...
    const result = await getPdfData(parsed.data, user);
