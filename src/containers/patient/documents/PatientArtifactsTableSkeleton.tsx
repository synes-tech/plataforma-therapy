...
interface PatientArtifactsTableSkeletonProps {
  rows?: number;
}

export function PatientArtifactsTableSkeleton({ rows = 5 }: PatientArtifactsTableSkeletonProps) {
  return (
    <div
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-2 bg-white/55 backdrop-blur-[1px]">
        <Spinner size="md" />
        <p className="text-xs font-medium text-charcoal-muted">Carregando documentos...</p>
      </div>
    </div>
  );
}
