...
export function CheckinDayDetailContent({ day }: CheckinDayDetailContentProps) {
  const mood = MOOD_LABELS[day.mood_score];
  const sleep = SLEEP_LABELS[day.sleep_quality];
  const dateFormatted = formatCheckinDateLong(day.date);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        {day.categories.length > 0 && (
          <div className="col-span-2 rounded-xl border border-slate-100 bg-slate-50/50 px-3 py-3 sm:col-span-1">
      ...
      <div className="rounded-xl border border-slate-100 bg-white px-4 py-4">
        ...
        {day.notes ? (
          <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-charcoal">{day.notes}</p>
        ) : (
