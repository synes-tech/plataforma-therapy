...
import { isSelfMode, usePortalContext } from '@shared/lib/portal-context';
    ...
    <div className="animate-fade-in w-full">
      <header className="mb-6 lg:mb-8">
        <h1 className="font-serif text-2xl tracking-tight text-charcoal lg:text-3xl">
          {selfMode ? 'Plano de cuidados' : 'Relatórios e Combinados'}
        </h1>
        <p className="mt-1 text-sm text-charcoal-muted lg:text-base">
          {selfMode
            ? 'Orientações do terapeuta, histórico de sessões e documentos do seu acompanhamento.'
            : `Orientações do terapeuta, histórico de sessões e documentos compartilhados com ${
                portal?.patient.first_name ?? data?.patient_name?.split(' ')[0] ?? 'o paciente'
              }.`}
        </p>
      </header>

      {selfMode && <PortalSubscriptionPanel />}
