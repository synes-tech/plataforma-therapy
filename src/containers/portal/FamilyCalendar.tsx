    ...
    <div className="animate-fade-in w-full space-y-6">
      <header>
        <h1 className="font-serif text-2xl tracking-tight text-charcoal lg:text-3xl">
          {selfMode ? 'Histórico' : 'Calendário de Rotina'}
        </h1>
        <p className="mt-1 text-sm text-charcoal-muted">
          {selfMode ? (
            'Suas sessões e os registros do seu dia.'
          ) : (
            <>
        ...
        >
          <div className="shrink-0 border-b border-slate-100 bg-gradient-to-r from-mint-50/40 to-white px-4 py-4 sm:px-5">
            <h2 className="font-display text-base font-semibold text-charcoal sm:text-lg">
              {selfMode ? 'Meus registros' : 'Diário de humor e rotina'}
            </h2>
            <p className="mt-0.5 text-xs text-charcoal-muted sm:text-sm">
              {selfMode
                ? 'O que você registrou neste mês — vários momentos no mesmo dia.'
                : 'Registros diários do portal — vários momentos no mesmo dia.'}
            </p>
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (canRegister) {
                            navigate(`/portal/diary?date=${cell.dateKey}`);
                          }
                        }}
