...
import { InviteCodeField } from '@containers/portal/invite-link/InviteCodeField';
import { InvitePatientSafetyCheck } from '@containers/portal/invite-link/InvitePatientSafetyCheck';
import { useInviteCodePreview } from '@containers/portal/invite-link/useInviteCodePreview';
...
/**
 * RegisterFamily — onboarding do portal (cuidador ou paciente).
...
export default function RegisterFamily() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await callPublicFunction<{ message: string }>('register-family', {
        name: name.trim(),
        email: email.trim(),
        password,
      ...
      const code = err && typeof err === 'object' && 'code' in err
        ...
        : '';
      const fallback = err instanceof Error ? err.message : 'Erro ao criar conta';
      setError(REGISTER_ERROR_OVERRIDES[code] || fallback);
    } finally {
  ...
  return (
    <div className="relative flex min-h-dvh">
        ...
        <div className="relative z-10 max-w-md px-12 xl:px-16">
          ...
          <blockquote className="text-center">
            <p className="font-serif text-2xl font-medium leading-snug tracking-tight text-charcoal xl:text-3xl [@media(max-height:800px)]:text-xl">
              Um espaço para acompanhar
              <br />
              o tratamento — <span className="italic text-primary-dark">seu</span> ou de quem
              você cuida.
            </p>
          </blockquote>
      ...
      {/* Formulário — coluna rolável. overflow-hidden + h-dvh travava o link "Entrar". */}
      <main className="relative z-10 w-full min-h-dvh overflow-y-auto overscroll-contain lg:w-[45%] lg:bg-white">
        <div className="mx-auto flex min-h-dvh w-full max-w-sm flex-col justify-center px-4 py-8 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-6 lg:px-8">
            {/* Logo mobile */}
