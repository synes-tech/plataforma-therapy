  ...
  return (
    <div className={className}>
      <button
        ...
        className={
          isIcon
            ? 'inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-charcoal transition-colors hover:border-primary/30 hover:text-primary disabled:cursor-not-allowed disabled:opacity-50'
            : isCompact
      ...
      >
        {loading ? (
          ...
          <>
            ...
            {isCompact && !isIcon ? 'PDF' : !isIcon ? 'Exportar PDF' : null}
          </>
        )}
      </button>
      {error && (
